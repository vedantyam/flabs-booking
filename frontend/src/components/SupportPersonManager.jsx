...
export default function SupportPersonManager() {
  ...
  async function loadPersons() {
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/api/support-persons`, { headers: authHeader() });
      setPersons(res.data || []);
    } catch {
  ...
  async function handleSave() {
    if (!form.name || !form.email || !form.work_start || !form.work_end) {
      alert('Name, email, work start, and work end are required');
      return;
    }
    setSaving(true);
    try {
      if (editId) {
        const res = await axios.put(`${API_URL}/api/support-persons/${editId}`, form, { headers: authHeader() });
        setPersons(p => p.map(x => x.id === editId ? res.data : x));
      } else {
        const res = await axios.post(`${API_URL}/api/support-persons`, form, { headers: authHeader() });
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-3">
          <h3 className="font-medium text-gray-800">{editId ? 'Edit Support Person' : 'Add Support Person'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Name *" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder="e.g. Tannu Sharma" />
            <Field label="Email *" type="email" value={form.email} onChange={v => setForm(f => ({ ...f, email: v }))} placeholder="e.g. tannu@example.com" />
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Breaks</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {persons.map(person => (
                <tr key={person.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-gray-600">{timeLabel(person.work_start)} – {timeLabel(person.work_end)}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">
                    {person.lunch_start && <div>Lunch: {timeLabel(person.lunch_start)}–{timeLabel(person.lunch_end)}</div>}
                    {person.tea_start && <div>Tea: {timeLabel(person.tea_start)}–{timeLabel(person.tea_end)}</div>}
                    {!person.lunch_start && !person.tea_start && '—'}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${person.is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {person.is_active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-3">
                      <button onClick={() => startEdit(person)} className="text-xs text-blue-600 hover:text-blue-800 font-medium transition">Edit</button>
                      <button onClick={() => handleDelete(person.id, person.name)} className="text-xs text-red-600 hover:text-red-800 font-medium transition">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
              {persons.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-center py-8 text-gray-400">No support persons added yet</td>
                </tr>
              )}
            </tbody>
          </table>
